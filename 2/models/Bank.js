export class Bank {
  ...
  deposit = (accontNumber, amount) => {
    for (let i = 0; i < this.scoreList.length; i++) {
      if (Object.keys(this.scoreList[i]) == accontNumber) {
        ...
        console.log(
          `Replenishment of the ${Object.keys(
            this.scoreList[i]
          )} account in the amount of ${Object.values(
            this.scoreList[i]
          )} Tugricks was completed successfully!`
        );
      }
    }
  };
  ...
  withdraw = (accontNumber, amount) => {
    for (let i = 0; i < this.scoreList.length; i++) {
      if (Object.keys(this.scoreList[i]) == accontNumber) {
        this.scoreList[i][accontNumber] -= amount;
        console.log(
          `The withdrawal of funds from the ${Object.keys(
            this.scoreList[i]
          )} account in the amount of ${Object.values(
            this.scoreList[i]
  ...
  checkBalance = (accontNumber) => {
    for (let i = 0; i < this.scoreList.length; i++) {
      if (Object.keys(this.scoreList[i]) == accontNumber) {
        console.log(
          `Deposit accaunt #${Object.keys(
            this.scoreList[i]
          )} balance is --> ${Object.values(this.scoreList[i])} Tugricks`
        );
      }
    }
  };
